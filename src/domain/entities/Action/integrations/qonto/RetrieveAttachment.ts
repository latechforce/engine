...
import type { QontoAttachment } from '../../../../integrations/Qonto/QontoTypes'
import {
  BaseQontoAction,
...
export interface RetrieveAttachmentQontoActionConfig extends BaseActionIntegrationConfig {
  attachmentId: string
}
...
export class RetrieveAttachmentQontoAction extends BaseQontoAction<Input, Output> {
  ...
  protected _process = async (input: Input) => {
    ...
    const attachment = await this._integrations.qonto.retrieveAttachment(
      account,
      input.attachmentId
    )
    if (!attachment) {
