...
export class LinkedinConnectionIntegration {
  public readonly tokenType = 'no-refresh-token'
  ...
  // LinkedIn does not provide refresh tokens in standard Marketing APIs
  async getAccessTokenFromRefreshToken(_refreshToken: string): Promise<Token> {
    throw new Error('LinkedIn refresh token flow is not supported')
  }
